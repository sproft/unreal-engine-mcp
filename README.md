...
| `pie_test_scene` | Scene-state assertion harness (minimum cut). Runs against the active editor world without driving Play in Editor and accepts a list of assertion specs each shaped `{kind, target, expected?, tolerance?}`. Two `kind` values are supported in this slice: `actor_exists` (target = actor name; pass = an actor with that `GetName()` or Outliner label is present) and `actor_at_location` (target = actor name, expected = `[x, y, z]` world-space location, optional `tolerance` = number, defaults 1.0 cm; pass = the resolved actor's `GetActorLocation` is within `tolerance` of `expected`). Per-assertion the response carries `index`, `kind`, `target`, `passed` flag, optional `actual` / `expected` / `delta` / `tolerance` for distance-based kinds, and a human-readable `message`. Aggregate counts (`total`, `passed`, `failed`, `unsupported`, `all_passed`) sit at the top of the response. The PIE-driven kinds (`var_equals` against a Blueprint instance variable, `actor_overlapping_tag` for overlap-driven gameplay assertions) ship in a later pass and stay on the backlog. |
